...
test('create user', async t => {
  ...
  await t.pressKey('enter')
  await t.expect(Selector('.fa-user').parent(0).innerText).contains('E2E TESTUSER')
})
...
test('select a group, signs in and gets added to the group', async t => {
  await t
    .click(Selector('.q-card').withText(`testgroup ${testTime}`).child('.fa-info-circle'))
    .click(Selector('.q-card-actions button'))
