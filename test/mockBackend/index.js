...
function createFinder(db, dbKey) {
  ...
  function filter(params) {
    return db[dbKey].filter((entry) =>
      Object.keys(params).every((field) => {
  ...
  function get(params, defaultValue) {
    ...
    if (entries.length > 1)
      throw new Error(
        `more than one entry! ${dbKey} for ${JSON.stringify(params)}`
      );
    if (entries.length === 0) {
  ...
  function sample(params) {
    ...
    if (entries.length === 0)
      throw new Error(
        `no entries found! ${dbKey} for ${JSON.stringify(params)}`
      );
